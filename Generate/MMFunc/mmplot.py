...
def saveSTFT_Multi(t, f, Zxx, save_path, wordIndex, fileIndex, personIndex, txIndex):
    num_subplots = 20
...
def saveSTFT_0824(t, f, Zxx, save_path, wordIndex, fileIndex, personIndex,
                  txIndex):
    num_subplots = 10
    t_intervals = np.linspace(t.min(), t.max(), num_subplots + 1)
    ...
    for i in range(num_subplots):
        ...
        if (i % 2 == 1):
            plt.savefig(
                f"{save_path}/{str(wordIndex)}_{str(fileIndex*5+i//2)}_{str(personIndex)}_{str(txIndex)}.png",
                bbox_inches='tight',
                pad_inches=0,
                dpi=300)
        else:
            plt.savefig(
                f"{save_path}/{str(wordIndex+1)}_{str(fileIndex*5+i//2)}_{str(personIndex)}_{str(txIndex)}.png",
                bbox_inches='tight',
                pad_inches=0,
                dpi=300)
        plt.close()

...
# 获取bin文件数据
data_raw = mmfunctions.getRawData_multiTx(
    bin_path, numSamples, numChirps, numFrames, numTx,numRx)  # numRx, numFrames, numSamples
...
for rx in range(4):
    # 1D-FFT
    data_1dfft = mmfunctions.rangeFFT(data_raw[tx][rx], numFrames, numSamples)
    # 去除静态杂波
    data_1dfft_dyn_all[rx, :, :] = mmfunctions.removeStatic(data_1dfft, numFrames, numSamples)
    # data_1dfft_dyn = data_1dfft

numAngles = 90
# 3D-FFT
data_3dfft = mmfunctions.angleFFT(data_1dfft_dyn_all, numFrames, numSamples,
                                  numAngles)

range_index, angle_index = mmfunctions.getRangeAngleIndex(data_3dfft)

# 提取相位并解缠绕
range_angle, angle_diff = mmfunctions.getUnwrapAngle(data_3dfft[:, range_index, angle_index])

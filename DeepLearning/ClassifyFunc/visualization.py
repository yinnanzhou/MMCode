...
def visualize_results(trainer):
    ...
    """
    # 绘制混淆矩阵
    cm = confusion_matrix([x for x in trainer.ground_truth],
                          [x for x in trainer.predictions])
    ...
    sns.heatmap(cm.astype('float') / cm.sum(axis=1)[:, np.newaxis],
                ...
                cmap='Blues',
                cbar=False)
    plt.title("Classification accuracy: {:.2%}".format(
        trainer.acces['test'][-1]))
    plt.xlabel("Predicted label")
    plt.ylabel("True label")
    # np.save('./classification/confusion_matrix/uwb_uwb_word_cm.npy', cm)
...
def visualize_predict(all_labels, all_preds):
    # Compute confusion matrix
    cm = confusion_matrix(all_labels, all_preds)

    # Calculate and print accuracy
    accuracy = sum([
        1 for true, pred in zip(all_labels, all_preds) if true == pred
    ]) / len(all_labels)
    ...
    sns.heatmap(cm.astype('float') / cm.sum(axis=1)[:, np.newaxis],
                ...
                cmap='Blues',
                cbar=False)
    # plt.title("Classification accuracy: {:.2%}".format(accuracy))
    plt.title("Classification accuracy: {:.2%}".format(accuracy))
    plt.xlabel("Predicted label")
    plt.ylabel("True label")
